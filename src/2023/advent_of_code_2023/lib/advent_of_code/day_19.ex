defmodule AdventOfCode2023.Day19 do
  ...
  def part1(lines) do
    ...
    for part <- part_list, valid_part <- valid_parts(part, rule_map["in"], rule_map), reduce: 0 do
      total ->
  ...
  defp parse_rule_stmt(stmt) do
    case Regex.split(~r/[<:>]/, stmt, include_captures: true) do
      [attrib, op, thresh, ":", dest] ->
        {String.to_existing_atom(op), String.to_existing_atom(attrib), String.to_integer(thresh),
         dest}

      # Just use a dummy action here:
      [else_dest] ->
